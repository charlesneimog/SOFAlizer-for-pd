...
windowsLibraries = ["ntdll.dll", "KERNEL32.DLL", "KERNELBASE.dll", "msvcrt.dll", "pd.dll", "KERNEL32.DLL"]
class localdeps:
    def __init__(self):
        ...
        if ".pd_linux" in self.args.pdobj or ".l_amd64" in self.args.pdobj:
            self.parse_linux()
        elif ".dll" in self.args.pdobj or ".m_amd64" in self.args.pdobj:
            self.parse_windows()
    ...
    def parse_windows(self):
        command = []
        command.append("ntldd")
        command.append(self.args.pdobj)
        ldd = subprocess.run(command, capture_output=True)
        for library_info in ldd.stdout.decode("utf-8").split("\n"):
            systemLibrary = False
            for sysLib in windowsLibraries:
                dynamiclibraryName = os.path.basename(library_info).lower()
                if sysLib.lower() in dynamiclibraryName or sysLib == self.args.pdobj:
                    systemLibrary = True
                    break   
            if systemLibrary:
                continue
            if "not found" in library_info:
                            ...
                            if os.path.exists(include):
                                for root, _, files in os.walk(include):
                                    for file in files:
                                        if file == libraryName:
                                            libraryFound = True
                                            if not os.path.exists(libraryName):
                                                shutil.copy(os.path.join(root, file), libraryName)
                        if not libraryFound:
                            print("Library not found: " + libraryName)
                            sys.exit(1)
            else:
                ...
                if match:
                    ...
                    if not os.path.exists(libraryName):
                        shutil.copy(library, libraryName)
